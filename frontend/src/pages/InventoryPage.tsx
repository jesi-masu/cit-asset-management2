...
const InventoryPage = () => {
  ...
  const filteredWorkstations = (
    selectedLabId
      ...
      : [...workstations]
  ) // Create a copy so we don't mutate state
            ...
            {user?.role === "Custodian" && availableLabs.length === 1 && (
              <div className="flex items-center space-x-2">
                ...
                <span className="px-2 py-1 text-xs font-medium bg-blue-100 text-blue-800 rounded-full">
                  {availableLabs[0]?.lab_name}{" "}
                  {availableLabs[0]?.location &&
                    `(${availableLabs[0]?.location})`}
                </span>
              </div>
            )}
            ...
            <table className="w-full">
              ...
              <tbody className="bg-white divide-y divide-gray-200">
                {filteredWorkstations.length === 0 ? (
                  ...
                  // 1. Create a copy with [...array] to avoid mutating state
                  // 2. Sort using localeCompare with { numeric: true }
                ...
                {filteredUnassignedAssets.length === 0 ? (
                  <tr>
                    <td
                      ...
                      className="px-6 py-4 text-center text-gray-500"
                    >
                      No unassigned assets found. All assets are assigned to
                      workstations.
                    </td>
                  </tr>
                ) : (
