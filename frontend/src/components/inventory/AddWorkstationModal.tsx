...
const AddWorkstationModal: React.FC<Props> = ({ show, onClose, onSuccess }) => {
  ...
  const handleAddToList = () => {
    ...
    const isDuplicate = workstations.some(
      (ws) => ws.workstation_name.toLowerCase() === name.trim().toLowerCase() && ws.lab_id === targetLabId
    );
  ...
  const handleSaveAll = async () => {
    ...
    try {
      ...
      if (err.response?.data?.error?.includes("Unique constraint")) {
        alert("Error: One of these workstation names already exists in this lab.");
      } else {
        ...
        <div className="flex items-center justify-center min-h-screen px-4">
          <div className="bg-white rounded-lg shadow-xl max-w-4xl w-full flex flex-col max-h-[90vh]">
            ...
            <div className="bg-gray-800 text-white px-6 py-4 rounded-t-lg flex justify-between items-center">
              ...
              <button onClick={handleClose} className="text-white hover:text-gray-300">✕</button>
            </div>
            ...
            <div className="p-6 overflow-y-auto">
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6 bg-gray-50 p-4 rounded border">
                <div className="md:col-span-2">
                  <label className="block text-sm font-medium text-gray-700 mb-1">Workstation Name *</label>
                  <input
                    type="text"
              ...
              <div className="border rounded overflow-hidden">
                <div className="bg-gray-100 px-4 py-2 border-b font-medium text-sm flex justify-between items-center">
                  ...
                  {workstations.length > 0 && <span className="text-xs text-blue-600">Click "Save All" to finish!</span>}
                </div>
                
                {workstations.length === 0 ? (
                  <div className="p-6 text-center text-gray-500 text-sm">
                    No workstations added yet. Use the form above to add them to this list.
                  </div>
                ) : (
                    ...
                    <table className="w-full text-sm text-left">
                      ...
                      <tbody className="divide-y">
                        {workstations.map((ws) => {
                          ...
                          return (
                            <tr key={ws.id}>
                              <td className="px-4 py-2 font-medium">{ws.workstation_name}</td>
                              <td className="px-4 py-2 text-gray-500">{lab?.lab_name || `Lab ID: ${ws.lab_id}`}</td>
